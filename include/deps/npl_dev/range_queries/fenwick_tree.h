...
fenwick_tree< T, Allocator >::_update ( size_type _n_, size_type _size_, value_type const & _x_ )
{
	...
	value_type current = _element_at( _n_++ );
        auto diff = _x_ - current;

	while( _n_ <= _size_ )
	{
		this->begin_[ _n_ - 1 ] += diff;
		_n_ += _p( _n_ );
	}
}
...
fenwick_tree< T, Allocator >::operator+ ( fenwick_tree< T, Allocator > const & _rhs_ ) const
{
        ...
        else
        {
                ...
                {
                        res.push_back( _rhs_.element_at( i ) );
                }
        }
...
fenwick_tree< T, Allocator >::operator- ( fenwick_tree< T, Allocator > const & _rhs_ ) const
{
        _self res( std::max< size_type >( size(), _rhs_.size() ) );
        ...
        else
        {
                ...
                for( size_type i = size(); i < _rhs_.size(); ++i )
                {
                        res.push_back( value_type() - _rhs_.element_at( i ) );
                }
        }
...
fenwick_tree< T, Allocator >::operator-= ( fenwick_tree< T, Allocator > const & _rhs_ ) noexcept
{
        if( size() > _rhs_.size() )
        {
                for( size_type i = 0; i < _rhs_.size(); ++i )
                {
                        _add( i, value_type() - _rhs_.element_at( i ) );
                }
        }
        else
        {
...
fenwick_tree< T, Allocator >::push_back ( value_type && _x_ )
{
	if( this->end_ < this->_end_cap() )
	{
		_construct_one_at_end( value_type() );
	}
	else
	{
	...
	_update( size() - 1, size(), NPL_FWD( _x_ ) );
}
