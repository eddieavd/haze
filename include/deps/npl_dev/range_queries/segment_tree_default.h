...
template< typename InputIterator,
	...
	>
segment_tree ( InputIterator, InputIterator )
	-> segment_tree< _iter_value_type< InputIterator >, Alloc >;

template< typename InputIterator,
	  typename Alloc,
	  typename = std::enable_if< _is_allocator< Alloc >::value >
	>
segment_tree ( InputIterator, InputIterator, Alloc )
	-> segment_tree< _iter_value_type< InputIterator >, Alloc >;
*/
...
segment_tree< T, PB, _pb_, Allocator >::_rebuild_tree () noexcept
{
	...
	{
		this->begin_[ i ] = this->pb_( this->begin_[ 2 * i ], this->begin_[ 2 * i + 1 ] );
	}
}
...
segment_tree< T, PB, _pb_, Allocator >::_restructure_tree_forward () noexcept
{
        ...
        {
                this->begin_[ capacity_ / 2 + i ] = this->begin_[ capacity_ / 4 + i ];
                this->end_++;
        }
}
...
template< typename T, parent_builder< T > PB, PB _pb_, typename Allocator >
typename segment_tree< T, PB, _pb_, Allocator >::size_type
segment_tree< T, PB, _pb_, Allocator >::_msb64 ( [[ maybe_unused ]] size_type _n_ ) const noexcept  //  TODO: this is hardcoded for 64bit addresses, make it general
{
	_n_ |= ( _n_ >>  1 );
	_n_ |= ( _n_ >>  2 );
	_n_ |= ( _n_ >>  4 );
...
template< typename T, parent_builder< T > PB, PB _pb_, typename Allocator >
typename segment_tree< T, PB, _pb_, Allocator >::size_type
segment_tree< T, PB, _pb_, Allocator >::_extend_to_pow2 ( [[ maybe_unused ]] size_type _n_ ) const noexcept
{
...
segment_tree< T, PB, _pb_, Allocator >::_update ( size_type _n_, const_reference _x_ )
{
	...
	{
		this->begin_[ _n_ ] = this->pb_( this->begin_[ 2 * _n_ ], this->begin_[ 2 * _n_ + 1 ] );
	}
}
...
<
	...
	void
>
segment_tree< T, PB, _pb_, Allocator >::_construct_at_end ( ForwardIterator _first_, ForwardIterator _last_, [[ maybe_unused ]] size_type _n_ )  //  hmmmmmmm
{
        ...
        : capacity_( 2 )
{
        ...
        _construct_at_end( capacity_ );
        this->end_ -= capacity_ / 2;
}
	...
	: _base( _alloc_traits::select_on_container_copy_construction( _x_._alloc() ) )
{
        ...
        if( new_size > 0 )
        {
                ...
                _construct_at_end( capacity_ / 2 - new_size );
                _rebuild_tree();

                this->end_ -= capacity_ / 2 - new_size;
        }
}
	...
	: _base( _a_ )
{
        ...
        if( _x_.size() > 0 )
        {
                ...
                _construct_at_end( _x_.begin(), _x_.end(), _x_.size() );
                _construct_at_end( capacity_ / 2 - _x_.size() );
                _rebuild_tree();

                this->end_ -= capacity_ / 2 - _x_.size();
        }
}
...
segment_tree< T, PB, _pb_, Allocator >::segment_tree ( std::initializer_list< value_type > _list_ )
{
	if( _list_.size() > 0 )
	{
		...
		_construct_at_end( size - _list_.size() );
		_rebuild_tree();

                this->end_ -= size - _list_.size();
	}
}
...
template< typename T, parent_builder< T > PB, PB _pb_, typename Allocator >
bool
segment_tree< T, PB, _pb_, Allocator >::operator== ( [[ maybe_unused ]] segment_tree< T, PB, _pb_, Allocator > const & _rhs_ ) const noexcept
{
	if( pb_ != _rhs_.pb_ )
	{
		return false;
...
template< typename T, parent_builder< T > PB, PB _pb_, typename Allocator >
auto
segment_tree< T, PB, _pb_, Allocator >::operator+ ( [[ maybe_unused ]] segment_tree< T, PB, _pb_, Allocator > const & _rhs_ ) const
{
	NPL_ASSERT( size() == _rhs_.size(), "segment_tree::operator+: size != rhs size" );

	segment_tree< T, PB, _pb_, Allocator > res( *this );

	for( size_type i = 0; i < size(); ++i )
	{
                res.update( i, res.at( i ) + _rhs_.at( i ) );
	}
...
template< typename T, parent_builder< T > PB, PB _pb_, typename Allocator >
auto &
segment_tree< T, PB, _pb_, Allocator >::operator+= ( [[ maybe_unused ]] segment_tree< T, PB, _pb_, Allocator > const & _rhs_ ) noexcept
{
	NPL_ASSERT( size() == _rhs_.size(), "segment_tree::operator+=: size != rhs size" );
...
{
//      NPL_ASSERT( !empty() && _x_ < size() && _y_ < size(), "segment_tree::_range: bad args" );
...
segment_tree< T, PB, _pb_, Allocator >::_push_back_slow_path ( U && _x_ )
{
        ...
        split_buffer< value_type, allocator_type & > b( capacity_ * 2, capacity_, a );
        capacity_ *= 2;
...
template< typename... Args >
void
segment_tree< T, PB, _pb_, Allocator >::_emplace_back_slow_path ( [[ maybe_unused ]] Args&&... _args_ )
{
	allocator_type & a = this->_alloc();
